...
Here we see that there were two values that are not numeric: "5+" and "Graduate student".  First, let's see how many people made each response.  To do this, we will introduce another Tidyverse function called `summarize()`.  This function takes a data frame and computes a summary across its rows.  In order to create a summary of the number of indviduals making each response, we need to first group the data by the variable of interest (*year* in this case), which we can do using the `group_by()` function.  If we then pass the grouped data to the summarize function, it will perform the summary separately for each different value of the variable.  To compute the number of observation in each of the categories, we can use the `n()` function, which simply counts the number of observatations.

```{r summarize_example, message=FALSE, exercise=TRUE, exercise.setup='survey-setup'}
# first group the data by year
survey_data_by_year = group_by(survey_data, year)

# then compute the number in each year category using n()
summarize(survey_data_by_year,
          n=n())
```
...
Now that we have the data filtered and grouped, we can compute the mean value for each group on each variable.  Once again we can use the `summarize()` function, but in this case we need to tell it to summarize across several variables.  We can do that using the `across()` function, telling it which variables to summarize across.


```{r across_example, exercise=TRUE, message=FALSE, exercise.setup='survey-setup'}
summarize(survey_data_by_year_filtered,
          across(.fns=mean))
```
...
summarize(survey_data_by_year_filtered_dropna,
          across(.fns=mean))
```
...
question("Which of the following are appropriate interpretations of this figure? Choose all that apply.",
  ...
  answer("Year 1 students expect to hate the class more than the other years", correct=TRUE),
  answer("Year 2-4 students all respond in a fairly similar way", correct = TRUE),
  answer("Year 1 students expect to love the class much more than the other years")
)
```

### Plotting a box plot

Another useful way to plot data across groups would be to use a *box plot*.  To to this, we just replace the geometry with `geom_boxplot()`.  In addition, for the aesthetics, we would use a grouping variable (i.e. "group=year") rather than a color variable ("color=year") as we did above.

#### Exercise

Complete the code in this box tenerate a box plot for the variable *statatt_atf* which represents the students' attitudes towards the field of statistics.  Whereever there are underscores ("___") you should fill them in with the appropriate commands.

```{r boxplot_exercise, message=FALSE, exercise=TRUE, exercise.setup='survey-setup'}

ggplot(survey_data_by_year_filtered_dropna,
       aes(y=______, group=______)) + 
  _______() + 
  xlab('Year') + 
...
- specify the x and y variables in the aesthestics
